 ...
 */
public class ScopeAttributePopulator extends AbstractScopeAttributeHandler {
    private static final Logger logger_ = Logger
            .getLogger(ScopeAttributePopulator.class);

    public ScopeAttributePopulator(String name, Scope scope,
    ...
    public void injectTo(Object component) {
        for (Iterator<String> itr = scope_.getAttributeNames(); itr.hasNext();) {
            ...
            try {
                Object value = adjustValue(scope_.getAttribute(name));
                typeConversionManager_.setProperty(component, name, value);
            } catch (Throwable t) {
                if (logger_.isDebugEnabled()) {
                    logger_.debug("Can't populate scope attribute: scope="
                            + scope_ + ", attribute name=" + name + ", value="
                            + value, t);
                }
            }
        }

...
public class HotdeployUtils extends HotdeployUtil {
     ...
     * ただしListインタフェースを実装している場合は、Hotdeploy由来かどうかに関わらず変換されます。</li>
     * <li>配列やListの場合は中身の要素も上記ルールに従って変換されます。</li>
    ...
    static void copy(Object source, Object destination) {
        ...
        } else if (List.class.isAssignableFrom(destinationClass)) {
            List sourceList = (List) source;
            List<Object> destinationList = (List) destination;
            for (Iterator itr = sourceList.iterator(); itr.hasNext();) {
                destinationList.add(fit(itr.next()));
            }
        } else {

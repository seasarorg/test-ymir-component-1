...
$.fn.editable = function(options){
	...
	options.toEditable = function(){
        ...
        val = this.innerHTML;
        $this.data('editable.original', val);
        
        if ($.isFunction(opts.onLoadText)) {
            val = opts.onLoadText.apply($this, [{
                original: $this.data('editable.original')
            }]);
        }
        $this.data('editable.current', val);
	...
	options.toNonEditable = function($this,change){
        var val = change ? $.editableFactory[opts.type].getValue($this,opts)
            : $this.data('editable.original');
        
        // Call User Function
        var preFunc = null;
        if($.isFunction(opts.onSubmitting)&&change==true)
            preFunc = opts.onSubmitting;
        else if($.isFunction(opts.onCancelling)&&change==false)
            preFunc = opts.onCancelling;
        if(preFunc!=null) {
            if (preFunc.apply($this,
                        [{
                            current:val,
                            previous:$this.data('editable.previous')
                        }]
		...
		$this.one(opts.editBy,opts.toEditable)
			 .data('editable.current', val).html(
				    opts.type=='password'
				   		?'*****'
...
$.editableFactory = {
	...
	'textarea': {
		toEditable: function($this,options){
            var textarea;
            if ($.isFunction(options.textareaFactory)) {
                textarea = options.textareaFactory.apply($this);
            } else {
                textarea = $('<textarea/>');
            }

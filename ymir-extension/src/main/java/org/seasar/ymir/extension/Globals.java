package org.seasar.ymir.extension;

public interface Globals extends org.seasar.ymir.Globals {
    String META_NAME_PROPERTY = "property";

    String META_NAME_FORMPROPERTY = "formProperty";

    String META_NAME_SOURCE = "source";

    String META_NAME_BORNOF = "bornOf";

    String PATH_PREFERENCES_DIRECTORY = ".settings";

    String ATTR_UNDECIDEDPARAMETERNAMES = "undecidedParameterNames";

    String ATTR_ACTION = "action";

    String ATTR_ACTION_INTERFACE = "action.interface";

    String ATTR_ACTION_INTERFACE_SHORTNAME = "action.interface.shortName";

    String ATTR_ACTION_KEY = "action.key";

    String ATTR_OWNERPAGE = "ownerPage";

    String ATTR_FORMDTO = "formDto";

    String ATTR_EVALUATEDBODY = "evaluatedBody";

    String PARAMETER_IMPORTCLASSSET = "importClassSet";

    String PARAMETER_BASEIMPORTCLASSSET = "baseImportClassSet";

    String PARAMETER_ENTITYMETADATA = "entityMetaData";

    String PARAMETER_TARGETCLASSDESC = "targetClassDesc";

    String PARAMETER_PAIRTYPEDESCS = "pairTypeDescs";

    String PARAMETER_AUXDESCLIST = "auxDescList";

    String PARAMETER_PREAMBLE = "preamble";

    String PARAMETER_CLASSDESC = "classDesc";

    String PARAMETER_IMPORTDESC = "importDesc";

    String BORNOFPREFIX_REQUEST = "request:";

    String ACTIONKEY_DEFAULT = "";
}
